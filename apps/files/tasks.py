...
def check_file_flake8_task() -> None:
    """Celery задача для проверки flake8 новых файлов и отправки уведомлений."""
...
def run_flake8(file_path: str) -> Optional[CompletedProcess]:
    ...
    try:
        return subprocess.run(['flake8', file_path], capture_output=True, text=True)
    except Exception as e:
        logger.error(f"Ошибка при проверке Flake8: {e}")
        return None


def create_file_check_log(file_upload: FileUpload, result: CompletedProcess) -> FileCheckLogs:
...
def compose_email_message(file_upload: FileUpload, result: CompletedProcess) -> str:
...
def update_email_cache(email_count: int, email_timestamp: Optional[datetime]) -> None:
